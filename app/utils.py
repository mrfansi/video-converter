...
def create_temp_directory() -> str:
    ...
    temp_dir = os.path.join(
        settings.TEMP_DIR, f"{int(time.time())}_{uuid.uuid4().hex[:8]}"
    )
    ...
    try:
        ...
        try:
            ...
            if blank_count > 3 or len(frame_files) == 0:
                logger.warning(
                    f"ffmpeg extraction produced {blank_count} blank frames out of 5 checked"
                )
                raise Exception("ffmpeg extraction produced blank frames")

            logger.info(
                f"Successfully extracted {len(frame_files)} frames using ffmpeg"
            )
            return frame_files
        ...
        for i in range(num_frames):
            ...
            if mean_val < 5 or std_val < 3:
                logger.warning(
                    f"Frame at position {frame_pos} appears blank (mean={mean_val}, std={std_val})"
                )
                continue
    ...
    try:
        ...
        for i in range(3):  # RGB channels
            ...
            if channel.max() > channel.min():
                pixels[:, :, i] = np.interp(
                    channel, (channel.min(), channel.max()), (0, 255)
                ).astype(np.uint8)
