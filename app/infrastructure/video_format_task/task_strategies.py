...
class StandardVideoFormatTaskStrategy(BaseVideoFormatTaskStrategy):
    ...
    def process_video_format_task(
        ...
        try:
            ...
            # Track progress through steps: Initialize, convert, upload
            current_step = 0
            ...
            if task_id:
                self.progress_tracker.update_progress(
                    ...
                    percent=10,
                    details="Analyzing video properties",
                )
            ...
            def progress_update(task_id, current_step, percent, details):
                if task_id:
                    self.progress_tracker.update_progress(
                        ...
                        percent=20
                        + int(percent * 0.7),  # Scale to 20-90% of overall progress
                        details=details,
                    )
            ...
            params_builder.with_format(VideoFormat(output_format))
            params_builder.with_quality(VideoQuality(quality))

            if width and height:
                # Use width and height directly since there's no Resolution constructor
                params_builder.with_param("width", width)
                params_builder.with_param("height", height)

            if bitrate:
                params_builder.with_param("bitrate", bitrate)
            ...
            if audio_codec:
                params_builder.with_codecs("libx264", audio_codec)

            if audio_bitrate:
                params_builder.with_param("audio_bitrate", audio_bitrate)

            # Add progress callback if task_id is provided
            if task_id:
                params_builder.with_progress_callback(
                    lambda progress: progress_update(
                        task_id,
                        "Converting video",
                        progress,
            ...
            logger.info(
                f"Video converted successfully: {conversion_result['output_path']}"
            )
            ...
            upload_result = self.cloud_uploader.upload_file(
                conversion_result["output_path"], content_type=f"video/{output_format}"
            )
            ...
            if not upload_result["success"]:
                ...
                raise Exception(
                    f"Failed to upload to Cloudflare R2: {upload_result.get('error', 'Unknown error')}"
                )
            ...
            if task_id:
                self.progress_tracker.update_progress(
                    ...
                    percent=100,
                    details="Video conversion completed successfully",
                )
...
class OptimizedVideoFormatTaskStrategy(BaseVideoFormatTaskStrategy):
    ...
    def process_video_format_task(
        ...
        try:
            logger.info(
                f"Converting video format with optimized strategy: {original_filename}"
            )

            # Use the standard strategy with additional optimizations
            standard_strategy = StandardVideoFormatTaskStrategy(
                self.progress_tracker, self.cloud_uploader
            )
...
class FallbackVideoFormatTaskStrategy(BaseVideoFormatTaskStrategy):
    ...
    def process_video_format_task(
        ...
        try:
            logger.info(
                f"Converting video format with fallback strategy: {original_filename}"
            )
            ...
            if task_id:
                self.progress_tracker.update_progress(
                    ...
                    percent=10,
                    details="Using simplified conversion approach for reliability",
                )
            ...
            logger.info(
                f"Video converted with fallback strategy: {conversion_result['output_path']}"
            )
            ...
            if task_id:
                self.progress_tracker.update_progress(
                    ...
                    current_step="Completed",
                    percent=100,
                    details="Video conversion completed with fallback strategy",
                )
