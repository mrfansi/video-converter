...
class FrameExtractor:
    ...
    def extract_frames(self, params: FrameExtractionParams) -> List[str]:
        ...
        """
        logger.info(f"Extracting frames from {params.input_path} with {params.extraction_method.value} strategy")
        
        # Select the appropriate strategy based on the extraction method
        if params.extraction_method == FrameExtractionMethod.FFMPEG and not isinstance(self.strategy, FFmpegExtractionStrategy):
            self.set_strategy(FFmpegExtractionStrategy(
                extra_params=params.extra_ffmpeg_params
            ))
        elif params.extraction_method == FrameExtractionMethod.OPENCV and not isinstance(self.strategy, OpenCVExtractionStrategy):
            self.set_strategy(OpenCVExtractionStrategy())
        elif params.extraction_method == FrameExtractionMethod.HYBRID and not isinstance(self.strategy, HybridExtractionStrategy):
            self.set_strategy(HybridExtractionStrategy(
                extra_ffmpeg_params=params.extra_ffmpeg_params
            ))
    ...
    @staticmethod
    def create_strategy(method: Union[FrameExtractionMethod, str], 
                        extra_ffmpeg_params: Optional[List[str]] = None) -> IFrameExtractionStrategy:
