...
class OpenCVImageTracer(IImageTracer):
    ...
    def trace_image(self, input_path: str, output_path: str, **options) -> str:
        ...
        logger.info(f"Traced {input_path} to {output_path} with {result.path_count} paths")
        return output_path
    
    def _trace_with_strategy(self, strategy: BaseImageTracingStrategy, image: np.ndarray, 
                           simplify_tolerance: float) -> TracingResult:
        ...
        try:
            ...
            logger.error(f"Error tracing image with strategy {strategy.__class__.__name__}: {str(e)}")
