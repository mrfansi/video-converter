...
class DominantColorStrategy(BaseColorExtractionStrategy):
    ...
    def _extract_colors(self, image: np.ndarray, contours: List[np.ndarray]) -> List[Tuple[int, int, int]]:
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("sklearn not available, using centroid color extraction instead")
        ...
        complexity_factor = min(max(int(pixel_std / 10), 3), self.max_colors)  # Between 3-max_colors
        ...
        try:
            ...
            contour_colors = self._assign_colors_to_contours(image, contours, rgb_colors)
            
            return contour_colors
        except Exception as e:
            logger.warning(f"Color clustering failed, using centroid color extraction instead: {str(e)}")
            centroid_strategy = CentroidColorStrategy()
            return centroid_strategy._extract_colors(image, contours)
    
    def _assign_colors_to_contours(self, image: np.ndarray, contours: List[np.ndarray], 
                                 dominant_colors: List[Tuple[int, int, int]]) -> List[Tuple[int, int, int]]:
        """Assign colors to contours based on position and size.
...
class HybridColorStrategy(BaseColorExtractionStrategy):
    ...
    def _extract_colors(self, image: np.ndarray, contours: List[np.ndarray]) -> List[Tuple[int, int, int]]:
        ...
        for i, contour in enumerate(contours):
            ...
            if i < len(dominant_colors) and i < len(centroid_colors) and i < len(average_colors):
