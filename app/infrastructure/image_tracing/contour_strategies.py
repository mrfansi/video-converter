...
class AdaptiveThresholdStrategy(BaseContourExtractionStrategy):
    ...
    def _extract_contours(self, image: np.ndarray) -> Tuple[List[np.ndarray], np.ndarray]:
...
class ColorBasedSegmentationStrategy(BaseContourExtractionStrategy):
    ...
    def _extract_contours(self, image: np.ndarray) -> Tuple[List[np.ndarray], np.ndarray]:
        ...
        # Find contours
        contours, _ = cv2.findContours(combined_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_TC89_KCOS)
...
class HybridContourStrategy(BaseContourExtractionStrategy):
    ...
    def _extract_contours(self, image: np.ndarray) -> Tuple[List[np.ndarray], np.ndarray]:
        ...
        adaptive_contours, adaptive_image = self.adaptive_strategy._extract_contours(image)
        ...
        logger.info(f"Hybrid strategy found {len(all_contours)} contours, filtered to {len(filtered_contours)}")
