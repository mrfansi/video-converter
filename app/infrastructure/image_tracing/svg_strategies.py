...
class EmbeddedImageSVGStrategy(BaseSVGGenerationStrategy):
    ...
    def _generate_svg_content(
        ...
        buffered = BytesIO()
        pil_img.save(
            buffered, format="PNG", optimize=self.optimize, quality=self.image_quality
        )
...
class AdaptiveSVGStrategy(BaseSVGGenerationStrategy):
    ...
    def _create_artificial_contours(self, image: np.ndarray) -> str:
        ...
        grid_size = base_grid_size + int(
            min(
                complexity / complexity_threshold * (max_grid_size - base_grid_size),
