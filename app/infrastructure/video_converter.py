...
class VideoConverter:
    ...
    def convert_video(self, params: VideoConversionParams) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Converting video {params.input_path} to {params.output_format} with {params.quality.value} quality")
        
        # Select the appropriate strategy based on the quality
        if params.quality == VideoQuality.HIGH and not isinstance(self.strategy, HighQualityVideoConversionStrategy):
            self.set_strategy(HighQualityVideoConversionStrategy())
        elif params.quality == VideoQuality.LOW and not isinstance(self.strategy, FastVideoConversionStrategy):
