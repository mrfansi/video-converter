...
class StandardVideoConversionStrategy(BaseVideoConversionStrategy):
    ...
    def convert_video(
        ...
        try:
            ...
            video_stream = self.video_scaler.scale_video(
                input_stream.video, width, height
            )

            # Build the output arguments
            output_args = {"c:v": video_codec, **video_params}

            # Add audio parameters if audio codec is specified
            if audio_codec:
                output_args.update({"c:a": audio_codec, **audio_params})
            ...
            if progress_callback and isinstance(progress_callback, Callable):
                ...
                if (
                    hasattr(progress_callback, "__code__")
                    and progress_callback.__code__.co_varnames
                ):
                    if progress_callback.__code__.co_varnames[0] == "task_id":
                        # This is a progress callback that expects a task_id
                        task_id = (
                            "task_id"  # Placeholder, will be provided by the caller
                        )
            ...
            if task_id and progress_callback and total_duration and total_duration > 0:
                ...
                self.progress_tracker.track_progress(
                    process, total_duration, task_id, progress_callback
                )
            ...
            return {
                ...
                "format": os.path.splitext(output_path)[1][
                    1:
    ...
    def convert_video(
        ...
        try:
            ...
            if video_codec == "libx264" or video_codec == "libx265":
                ...
                high_quality_video_params["crf"] = min(
                    high_quality_video_params.get("crf", 23), 18
                )
                # Use slower preset for better compression
                high_quality_video_params["preset"] = "slow"
            elif video_codec == "libvpx-vp9":
                high_quality_video_params["crf"] = min(
                    high_quality_video_params.get("crf", 31), 24
                )
            ...
            if audio_codec == "aac":
                high_quality_audio_params["b:a"] = high_quality_audio_params.get(
                    "b:a", "192k"
                )
            ...
            return standard_strategy.convert_video(
                ...
                video_codec,
                audio_codec,
                high_quality_video_params,
                high_quality_audio_params,
                width,
                height,
                progress_callback,
            )
