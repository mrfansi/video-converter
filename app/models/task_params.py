...
class TaskParams(BaseParams):
    ...
    updated_at: Optional[datetime] = Field(default=None, description="Task last update timestamp")
...
class TaskParamBuilder(ParamBuilder[TaskParams]):
    ...
    def with_timestamps(self, created_at: datetime, updated_at: Optional[datetime] = None) -> 'TaskParamBuilder':
...
class TaskUpdateParams(BaseParams):
    ...
    status: Optional[TaskStatus] = Field(default=None, description="New status of the task")
    progress: Optional[int] = Field(default=None, description="New progress percentage (0-100)")
    result: Optional[Dict[str, Any]] = Field(default=None, description="Task result data")
    error: Optional[str] = Field(default=None, description="Error message if task failed")
    ...
    def has_updates(self) -> bool:
        ...
        """
        return any(v is not None for v in [self.status, self.progress, self.result, self.error])
