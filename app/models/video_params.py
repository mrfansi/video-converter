...
class VideoConversionParams(BaseParams):
    ...
    quality: VideoQuality = Field(default=VideoQuality.MEDIUM, description="Video quality preset")
    resolution: Optional[VideoResolution] = Field(default=None, description="Output video resolution")
    ...
    extra_ffmpeg_params: Optional[List[str]] = Field(default=None, description="Additional ffmpeg parameters")
...
class VideoConversionParamBuilder(ParamBuilder[VideoConversionParams]):
    ...
    def with_format(self, format: Union[VideoFormat, str]) -> 'VideoConversionParamBuilder':
        """Set the output format."""
        if isinstance(format, str):
            format = VideoFormat(format)
        return self.with_param("output_format", format)
    
    def with_quality(self, quality: Union[VideoQuality, str]) -> 'VideoConversionParamBuilder':
        """Set the video quality."""
        if isinstance(quality, str):
            quality = VideoQuality(quality)
        return self.with_param("quality", quality)
    
    def with_resolution(self, resolution: Union[VideoResolution, str]) -> 'VideoConversionParamBuilder':
    ...
    def with_time_range(self, start_time: float, end_time: float) -> 'VideoConversionParamBuilder':
        """Set the video time range."""
        self.with_param("start_time", start_time)
        return self.with_param("end_time", end_time)
    
    def with_codecs(self, video_codec: str, audio_codec: str = None) -> 'VideoConversionParamBuilder':
    ...
    def with_bitrates(self, video_bitrate: str, audio_bitrate: str = None) -> 'VideoConversionParamBuilder':
    ...
    def with_extra_ffmpeg_params(self, params: List[str]) -> 'VideoConversionParamBuilder':
        """Set additional ffmpeg parameters."""
        return self.with_param("extra_ffmpeg_params", params)
    
    def with_progress_callback(self, callback: Callable[[int], None]) -> 'VideoConversionParamBuilder':
...
class FrameExtractionParams(BaseParams):
    ...
    height: Optional[int] = Field(default=None, description="Height to resize frames to")
    ...
    start_time: Optional[float] = Field(default=None, description="Start time in seconds")
    end_time: Optional[float] = Field(default=None, description="End time in seconds")
    frame_prefix: str = Field(default="frame_", description="Prefix for frame filenames")
    frame_format: str = Field(default="png", description="Format for extracted frames")
    
    # Advanced options
    blank_threshold_mean: float = Field(default=5.0, description="Mean pixel value threshold for blank frame detection")
    blank_threshold_std: float = Field(default=3.0, description="Standard deviation threshold for blank frame detection")
    extra_ffmpeg_params: Optional[List[str]] = Field(default=None, description="Additional ffmpeg parameters")
...
class FrameExtractionParamBuilder(ParamBuilder[FrameExtractionParams]):
    ...
    def with_extraction_method(self, method: Union[FrameExtractionMethod, str]) -> 'FrameExtractionParamBuilder':
    ...
    def with_time_range(self, start_time: float, end_time: float) -> 'FrameExtractionParamBuilder':
        """Set the video time range."""
        self.with_param("start_time", start_time)
        return self.with_param("end_time", end_time)
    
    def with_frame_naming(self, prefix: str, format: str) -> 'FrameExtractionParamBuilder':
        """Set the frame naming options."""
        self.with_param("frame_prefix", prefix)
        return self.with_param("frame_format", format)
    
    def with_blank_detection(self, mean_threshold: float, std_threshold: float) -> 'FrameExtractionParamBuilder':
    ...
    def with_extra_ffmpeg_params(self, params: List[str]) -> 'FrameExtractionParamBuilder':
        """Set additional ffmpeg parameters."""
        return self.with_param("extra_ffmpeg_params", params)
    
    def with_progress_callback(self, callback: Callable[[int, int], None]) -> 'FrameExtractionParamBuilder':
        """Set the progress callback function."""
        return self.with_param("progress_callback", callback)
