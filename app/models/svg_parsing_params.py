...
class SVGParsingParams(BaseModel):
    ...
    ignore_transforms: bool = Field(
        default=False, description="Whether to ignore transforms in the SVG"
    )
    extract_colors: bool = Field(
        default=True, description="Whether to extract colors from the SVG"
    )
    max_paths: Optional[int] = Field(
        default=None, description="Maximum number of paths to extract (None = no limit)"
    )
    error_handling: str = Field(
        default="raise", description="How to handle errors: 'raise', 'log', or 'ignore'"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional metadata for the SVG parsing process"
    )
