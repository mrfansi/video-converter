...
class VideoFormatTaskParams(BaseModel):
    ...
    output_format: str = Field(..., description="Desired output format (mp4, webm, etc.)")
    quality: str = Field("medium", description="Quality preset (low, medium, high, veryhigh)")
    width: Optional[int] = Field(None, description="Output width")
    height: Optional[int] = Field(None, description="Output height")
    bitrate: Optional[str] = Field(None, description="Video bitrate (e.g., \"1M\" for 1 Mbps)")
    preset: str = Field("medium", description="Encoding preset (ultrafast to veryslow)")
    crf: Optional[int] = Field(None, description="Constant Rate Factor (0-51, lower means better quality)")
    audio_codec: Optional[str] = Field(None, description="Audio codec (aac, mp3, opus, etc.)")
    audio_bitrate: Optional[str] = Field(None, description="Audio bitrate (e.g., \"128k\")")
    original_filename: Optional[str] = Field(None, description="Original filename of the uploaded video")
    task_id: Optional[str] = Field(None, description="Task ID for progress tracking")
    strategy: VideoFormatTaskStrategy = Field(VideoFormatTaskStrategy.STANDARD, description="Strategy to use for processing")
    progress_callback: Optional[Callable] = Field(None, description="Callback function for progress updates")
...
class VideoFormatTaskParamBuilder:
    ...
    def with_dimensions(self, width: Optional[int] = None, height: Optional[int] = None) -> 'VideoFormatTaskParamBuilder':
        """Set the dimensions."""
    ...
    def with_original_filename(self, original_filename: str) -> 'VideoFormatTaskParamBuilder':
    ...
    def with_strategy(self, strategy: Union[VideoFormatTaskStrategy, str]) -> 'VideoFormatTaskParamBuilder':
        """Set the strategy."""
    ...
    def with_progress_callback(self, progress_callback: Callable) -> 'VideoFormatTaskParamBuilder':
