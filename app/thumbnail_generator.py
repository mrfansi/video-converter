...
def generate_thumbnail_from_frame(
    ...
    try:
        ...
        if source_dimensions is None:
            ...
            logger.info(
                f"Using image dimensions for thumbnail: {source_dimensions[0]}x{source_dimensions[1]}"
            )
        else:
            logger.info(
                f"Using provided source dimensions for thumbnail: {source_dimensions[0]}x{source_dimensions[1]}"
            )
        ...
        if maintain_aspect_ratio:
            ...
            logger.info(
                f"Calculated thumbnail size: {thumb_width}x{thumb_height} (maintaining aspect ratio)"
            )
        else:
        ...
        logger.info(
            f"Generated thumbnail at {thumbnail_path} with dimensions {thumb.size[0]}x{thumb.size[1]}"
        )
...
def generate_thumbnail_from_lottie(
    lottie_json: Dict[str, Any], output_dir: str, size: Tuple[int, int] = (400, 400)
) -> str:
    """
    ...
    try:
        ...
        # Draw a border
        draw.rectangle(
            [(0, 0), (width - 1, height - 1)], outline=(200, 200, 200), width=2
        )
...
def upload_thumbnail(
    thumbnail_path: str, uploader: Any, object_key: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        result = uploader.upload_file(
            thumbnail_path, content_type="image/png", custom_key=thumbnail_key
        )
