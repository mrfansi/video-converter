...
# These imports are used to check if the lottie library is available
# They may appear unused by linters, but they're needed for feature detection
try:
    # We import these to verify the library is available with all required components
    # Even if not directly used in this file, they're imported to ensure
    # the complete API is available when LOTTIE_AVAILABLE is set to True
    import lottie  # noqa: F401
    from lottie import objects  # noqa: F401
    from lottie.parsers.svg import parse_svg_file  # noqa: F401
    from lottie.exporters.core import export_lottie  # noqa: F401
    from lottie.objects import ShapeElement, Group, Fill, Stroke, Path  # noqa: F401
    from lottie.objects import easing  # noqa: F401
    from lottie import NVector  # noqa: F401

    LOTTIE_AVAILABLE = True
    logger.info("python-lottie library available, using it for Lottie generation")
except ImportError:
    logger.warning(
        "python-lottie library not available, falling back to manual JSON generation"
    )
...
class LottieGenerator(ILottieGenerator):
    ...
    def create_lottie_animation(
        ...
        try:
            ...
            logger.info(
                f"Creating Lottie animation with {len(frame_paths)} frames, dimensions: {width}x{height}"
            )
    ...
    def save_lottie_json(
        self, lottie_json: Dict[str, Any], output_path: str, compress: bool = True
    ) -> str:
        """
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to calculate JSON size: {str(e)}")
                pre_size = 0
            ...
            with open(output_path, "w") as f:
                if compress:
                    json.dump(
                        lottie_json, f, separators=(",", ":"), cls=LottieJSONEncoder
                    )  # Remove whitespace
                else:
            ...
            if compress:
                compression_ratio = (
                    (1 - (post_size / pre_size)) * 100 if pre_size > 0 else 0
                )
                logger.info(
                    f"Compressed Lottie JSON from {pre_size} to {post_size} bytes ({compression_ratio:.2f}% reduction)"
                )
    ...
    def create_lottie_animation(
        ...
        try:
            ...
            if original_frame_count > max_frames and optimize:
                ...
                logger.info(
                    f"Sampled {len(sampled_frame_paths)} frames from {original_frame_count} original frames"
                )
            ...
            logger.info(
                f"Created Lottie animation manually with {len(sampled_frame_paths)} frames"
            )
            return lottie_json
