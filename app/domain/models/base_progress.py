...
class BaseProgressTracker(IProgressTracker):
    ...
    def update(
        self, task_id: str, current_step: int, message: Optional[str] = None
    ) -> None:
        ...
        if progress_data.status not in ["pending", "processing"]:
            logger.warning(
                f"Attempting to update progress for {progress_data.status} task: {task_id}"
            )
            return
    ...
    def _notify_callbacks(
        self, task_id: str, is_complete: bool = False, is_error: bool = False
    ) -> None:
