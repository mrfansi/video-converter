...
from app.domain.interfaces.converter import IConverter, IVideoConverter, ILottieGenerator, IImageTracer, ConversionResult
...
class BaseConverter(IConverter, ABC):
    ...
    def can_convert(self, source_format: str, target_format: str) -> bool:
        ...
        """
        return (source_format.lower(), target_format.lower()) in self._supported_conversions
...
class BaseVideoConverter(BaseConverter, IVideoConverter, ABC):
    ...
    def convert(self, params: VideoConversionParams) -> ConversionResult:
        ...
        if not self._validate_input_file(params.input_path):
            return ConversionResult.error_result(f"Invalid input file: {params.input_path}")
        
        # Ensure output directory exists
        if not self._ensure_output_directory(params.output_path):
            return ConversionResult.error_result(f"Failed to create output directory for: {params.output_path}")
...
class BaseLottieGenerator(BaseConverter, ILottieGenerator, ABC):
    ...
    def generate_from_frames(self, params: LottieAnimationParams) -> ConversionResult:
        ...
        if not os.path.exists(params.frames_dir):
            return ConversionResult.error_result(f"Input frames directory does not exist: {params.frames_dir}")
        
        if not os.path.isdir(params.frames_dir):
            return ConversionResult.error_result(f"Input frames path is not a directory: {params.frames_dir}")
    ...
    def generate_from_svgs(self, params: SVGConversionParams) -> ConversionResult:
        ...
        if not os.path.exists(params.svg_dir):
            return ConversionResult.error_result(f"Input SVG directory does not exist: {params.svg_dir}")
        
        if not os.path.isdir(params.svg_dir):
            return ConversionResult.error_result(f"Input SVG path is not a directory: {params.svg_dir}")
    ...
    @abstractmethod
    def _perform_frames_generation(self, params: LottieAnimationParams) -> ConversionResult:
