...
class BaseStorage(IStorage, ABC):
    ...
    def upload(self, file_path: str, content_type: str = None, custom_key: Optional[str] = None) -> StorageResult:
    ...
    def download(self, key: str, destination: str) -> StorageResult:
        ...
        if destination_dir:
            try:
                ...
                logger.error(f"Failed to create destination directory: {destination_dir}. Error: {str(e)}")
                return StorageResult.error_result(f"Failed to create destination directory: {str(e)}")
