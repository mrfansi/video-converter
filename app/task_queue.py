...
class TaskQueue:
    ...
    def __init__(self, workers: int = 2):
        ...
        self.storage_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "tasks")
    ...
    def update_progress(self, task_id: str, current_step: str, percent: int = None, 
        ...
        if completed_steps is not None:
            ...
            if percent is None and task.progress["total_steps"] > 0:
                task.progress["percent"] = int((completed_steps / task.progress["total_steps"]) * 100)
        ...
        logger.info(f"Updated progress for task {task_id}: {current_step} ({task.progress['percent']}%)")
