...
def convert_video_to_lottie(video_path, output_dir=None, fps=10, width=640, height=480, max_frames=50):
    """
    ...
    try:
        ...
        frame_paths = extract_frames(video_path, frames_dir, fps=fps, width=width, height=height)
        extract_end = time.time()
        results["timings"]["extract_frames"] = extract_end - extract_start
        results["stats"]["total_frames"] = len(frame_paths)
        logger.info(f"Extracted {len(frame_paths)} frames in {results['timings']['extract_frames']:.2f} seconds")
        ...
        results["timings"]["prepare_frames_avg"] = sum(prepare_times) / len(prepare_times) if prepare_times else 0
        results["timings"]["trace_svg_avg"] = sum(trace_times) / len(trace_times) if trace_times else 0
        results["stats"]["valid_paths_avg"] = sum(valid_path_counts) / len(valid_path_counts) if valid_path_counts else 0
        ...
        logger.info(f"Conversion completed in {results['timings']['total']:.2f} seconds")
        logger.info(f"Lottie JSON saved to {lottie_path} ({results['stats']['lottie_size_kb']:.2f} KB)")
...
def analyze_results(results):
    ...
    print(f"Frames: {results['stats']['total_frames']} (processed: {results['stats']['processed_frames']})")
    ...
    print(f"Average frame preparation: {results['timings']['prepare_frames_avg'] * 1000:.2f} ms/frame")
    print(f"Average SVG tracing: {results['timings']['trace_svg_avg'] * 1000:.2f} ms/frame")
...
def visualize_results(results):
    ...
    estimated_video_size = results['stats']['processed_frames'] * 30  # KB (rough estimate)
