...
def extract_frames_debug(video_path, output_dir, num_frames=10):
    ...
    for i in range(num_frames):
        ...
        if mean_value < 5 or mean_value > 250 or std_value < 5:
            logger.warning(f"Frame {i} appears to be blank: mean={mean_value}, std={std_value}")
            is_blank = True
        ...
        if is_blank:
            ...
            cv2.putText(annotated_frame, f"BLANK FRAME: mean={mean_value:.1f}, std={std_value:.1f}", 
                       (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            annotated_file = os.path.join(output_dir, f"debug_frame_{i:04d}_annotated.png")
            cv2.imwrite(annotated_file, annotated_frame)
        
        logger.info(f"Extracted frame {i} at position {frame_pos}: {'BLANK' if is_blank else 'OK'}")
