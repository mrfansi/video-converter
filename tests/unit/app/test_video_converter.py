...
class TestVideoConverter(BaseTest):
    ...
    def test_convert_video_basic(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
    ...
    def test_convert_video_with_different_formats(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
    ...
    def test_convert_video_with_quality_settings(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
        ...
        for quality in qualities:
            ...
            ffmpeg_cmd = mock_subprocess.Popen.call_args[0][0]
            if quality == "low":
                assert any("-crf 28" in " ".join(arg) for arg in zip(ffmpeg_cmd, ffmpeg_cmd[1:]))
            elif quality == "medium":
                assert any("-crf 23" in " ".join(arg) for arg in zip(ffmpeg_cmd, ffmpeg_cmd[1:]))
            elif quality == "high":
                assert any("-crf 18" in " ".join(arg) for arg in zip(ffmpeg_cmd, ffmpeg_cmd[1:]))
    
    def test_convert_video_with_resolution(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
        ...
        ffmpeg_cmd = mock_subprocess.Popen.call_args[0][0]
        assert any("-s 1280x720" in " ".join(arg) for arg in zip(ffmpeg_cmd, ffmpeg_cmd[1:]))
    
    def test_convert_video_with_framerate(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
    ...
    def test_convert_video_with_progress_callback(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
        ...
        mock_process.stdout.readline.side_effect = [line + b"\n" for line in progress_output.split(b"\n")] + [b""]
        ...
        with patch("app.video_converter.subprocess.Popen", return_value=mock_process):
            ...
            # Assertions
            assert result["success"] is True
            assert mock_callback.call_count >= 3  # Should be called for each progress line
    ...
    def test_convert_video_ffmpeg_error(self, temp_output_dir, mock_subprocess, mock_os, mock_shutil):
